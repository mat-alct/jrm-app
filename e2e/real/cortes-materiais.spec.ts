...
test.describe('cortes — materiais', () => {
  test('lista o material do seed', async ({ page }) => {
    ...
    await expect(page.getByText('MDF Branco').first()).toBeVisible();
  });

  test('cria um material novo e ele é persistido', async ({ page, adminDb }) => {
    await loginAs(page, 'admin');
    await page.goto('/cortes/materiais');
    ...
    await dialog.getByLabel('Material').fill('MDF Preto');
    ...
    await expect(page.getByText('MDF Preto')).toBeVisible();
  ...
  test('remove um material', async ({ page, adminDb }) => {
    ...
    await expect(page.getByText('MDF Branco')).toHaveCount(0);
    await expect
      .poll(async () => (await adminDb.collection('materials').get()).size)
      .toBe(0);
  });
  ...
  test('o preço do material alimenta o cálculo do novo serviço', async ({
    ...
    await page
      ...
      .first()
      .click();
    await page.getByText('Pago', { exact: true }).filter({ visible: true }).first().click();
