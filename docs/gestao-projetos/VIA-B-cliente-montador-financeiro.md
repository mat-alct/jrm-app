...
- `src/pages/montador/item/[projectId]/[itemId].tsx`: telefone clicável (`tel:`), endereço com link para mapa, arquivos técnicos (visibilidade `assembler`), botão grande **Atualizar etapa** (só transições permitidas ao montador: `em_producao` → `pronto_para_montagem` → `montagem_concluida` (Fase 2), via `status.service` da fundação), upload de fotos (attachment.service da fundação, captura de câmera).
