...
> **Estado atual (2026-07-17):** as seções 1–10 preservam o histórico das primeiras
> entregas. O fluxo vigente após o plano do vendedor está consolidado na **seção 11** e
> em [PLANO-FLUXO-VENDEDOR.md](../../PLANO-FLUXO-VENDEDOR.md).
...
| `src/pages/projetos/novo.tsx`                       | A   | Criar cadastro básico (nome + telefone)                                 |
...
| `src/pages/projetos/index.tsx`                      | A   | Aba compartilhada "Desenhos pendentes" (admin/desenhista)               |
...
| `src/pages/projetos/**`, `src/pages/administracao/{usuarios,configuracoes-prazos}.tsx`, `src/pages/api/admin/**`, `src/components/projects/**`, `src/components/designer/**`, `src/services/projects/{project,projectItem,designer}.service.ts`, sidebar                             | **Via A**            |
...
- `src/types/projects.ts`: `AppUser`, `UserRole`, `Project`, `ProjectItem`, `ProjectItemStatus`, `ClientApprovalStatus`, `Attachment`, `AttachmentAudience`, `ProjectNotification`, `ProjectItemVersion`, `StatusHistory`, `AssemblerAssignment`, `AssemblerPaymentStatus`, `AssemblerPayment`, `DeadlineDefaults`, DTOs do cliente (seções 6–14 e 20 da spec).
...
- `scripts/seed-projetos.mjs`: cria usuários de cada papel + 2 projetos com itens em status variados, assignments e notificação pendente — insumo para teste manual das duas vias.
...
WhatsApp/e-mail automático, assinatura digital, chat interno, cálculo automático de preço e comissão de vendedores. Notificações internas de pedido de informação foram incorporadas na seção 11.
...
### 10.2 Nova máquina de status (14 status após a consolidação de 2026-07-17)
...
aguardando_desenho              → aguardando_orcamento
    ...
    (desenhista informa justificativa; vendedor recebe notificacao)
aguardando_informacoes          → aguardando_desenho
    (vendedor/admin anexa o que falta e reaprova; prazo e recalculado)
...
**Correção: parar de mandar o telefone para o Auth.** O login é e-mail/senha; o campo `phoneNumber` do Auth existe para _phone sign-in_ e é **globalmente único por projeto** — dois funcionários cadastrados com o telefone da loja quebrariam com `auth/phone-number-already-exists`. O telefone é dado de exibição (link `tel:` do montador), então pertence a `users/{uid}` e a mais lugar nenhum.
...
**Validação da 2.0 (obrigatória antes de seguir):** criar usuário de cada papel pela UI e abrir `/administracao/financeiro-montadores`, `/montador` e `/projetos` (aba Desenhos pendentes) sem erro de índice.
...
- `src/types/projects.ts`: novo union `ProjectItemStatus` (14 valores da seção 10.2).
...
## 11. Consolidação do fluxo do vendedor, desenho e audiência (2026-07-17)

Esta seção substitui, para comportamento atual, as descrições históricas conflitantes das
seções anteriores.

### 11.1 Cadastro e itens

- `/projetos/novo` cria o cadastro apenas com nome e telefone e redireciona para o detalhe.
- E-mail e endereço são opcionais e editáveis em **Visão geral**; endereço passa a ser
  obrigatório somente na atribuição do montador.
- Projeto pode existir vazio. Itens são adicionados pela aba **Itens** do detalhe.
- Não existem anexos gerais do projeto: todo arquivo pertence a um item.

### 11.2 Anexos por audiência

`AttachmentAudience` possui `seller`, `designer`, `assembler` e `client`. As quatro flags
nascem `true`; o upload permite desmarcar públicos e admin pode editar depois. UI,
Firestore e Storage aplicam as mesmas decisões. O portal do cliente recebe somente
`audience.client === true`.

### 11.3 Aprovação e fila compartilhada de desenho

- Vendedor/admin clica em **Aprovar para desenho**; isso calcula `deadlineCurrent`, muda
  `projeto_criado → aguardando_desenho` e não escolhe desenhista.
- `/projetos`, aba **Desenhos pendentes**, mostra toda a fila a admin/desenhista. A rota
  `/desenhista` foi removida.
- Desenhista usa claim transacional (**Assumir**). Admin atribui por Renato, Marcio ou
  nome livre em Outros; se houver usuário designer ativo com o mesmo nome, `designerId`
  também é gravado.
- Somente o desenhista que assumiu envia versão.

### 11.4 Pedido de informações e notificações

`aguardando_desenho → aguardando_informacoes` exige justificativa. A ação cria
`projects/{projectId}/notifications/{id}` e mantém o desenhista. O detalhe do projeto tem
abas **Visão geral**, **Itens** e **Notificações**, com badge das pendentes. Após o vendedor
anexar o que falta e reaprovar, o prazo é recalculado, a notificação é resolvida e o item
volta à fila atribuído à mesma pessoa. Para o cliente, ambos os status usam o rótulo neutro
**Projeto em desenvolvimento**.

### 11.5 Corte de visão do vendedor

- Vendedor vê todos os cadastros na lista; `listProjects({ sellerId })` continua opcional
  para filtros administrativos.
- `assignAssemblers` grava `ProjectItem.assemblerAssignedAt`.
- A partir desse instante, um usuário cujo único papel é `seller` vê no detalhe apenas
  nome, ambiente e status. Queries de anexos, versões, histórico e assignments não são
  disparadas; Firestore e Storage negam anexos/versões.
- Admin nunca é bloqueado. Usuários com papéis acumulados preservam as permissões do papel
  operacional adicional.
- Limitação conhecida: Firestore não filtra campos do documento do item; o seller ainda
  pode ler o doc para obter o status. A confidencialidade de anexos/versões é aplicada nas
  subcoleções e no Storage, enquanto orçamento/demais campos são reduzidos na UI.

### 11.6 Validação automatizada

O fluxo é coberto por matrizes independentes de status/permissões, specs unitários e de
componentes, regras reais no Emulator Suite, integração dos services e e2e com dupla
verificação no emulador. A contagem e o resultado final ficam registrados em
[PLANO-FLUXO-VENDEDOR.md](../../PLANO-FLUXO-VENDEDOR.md), fase 9.
